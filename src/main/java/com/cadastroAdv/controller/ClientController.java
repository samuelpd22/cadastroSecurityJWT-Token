...
public class ClientController {
    ...
    public ResponseEntity<String> cadastrarCliente(@RequestBody ClientDTO clientDTO) {
    ...
    public ResponseEntity<String> editarporId(@PathVariable Long id, @RequestBody ClientDTO clientDTO){
        Optional<ClientEntity> clientExist = clientRepository.findById(id);
        if(!clientExist.isPresent()){
            throw new EntityNotFoundException("Pessoa não encontrada");
        } else {
            ClientEntity newClient = clientExist.get();
            BeanUtils.copyProperties(clientDTO, newClient);
            clientRepository.save(newClient);
